...
namespace Canvas
{
    ...
    //
    // Per-draw world placement comes from the paired mesh's
    // GetLocalBounds + the owning node transform.
    ...
    XGfxMeshData : public XCanvasElement
    {
        ...
        //
        // For displaced meshes the engine grows the pre-displacement
        // bounds outward symmetrically in all three axes by the
        // material's worst-case displacement magnitude, so cull and
        // shadow-frustum callers see the displaced geometry envelope
        // without needing to reason about displacement themselves.
        //
    ...
    struct MeshDataDesc
    {
        ...
        // Topology for all groups of this mesh.  Patch topologies feed the
        // displaced-mesh pipeline when paired with a displacement-equipped
        // material; the engine generates draws from these groups' position
        // and UV0 streams.
        ...
        // and becomes the mesh's stored bounds verbatim.  Intended for
        // meshes whose drawn geometry covers a known extent that is
        // not fully described by any CPU position buffer (e.g. a patch
        // grid whose CPs sit at corner samples but whose interior is
        // generated by tessellation).
        //
        // For displaced meshes supply the pre-displacement CP envelope;
        // GetLocalBounds inflates outward by the paired material's
        // displacement magnitude automatically.
    ...
    struct XGfxDevice : public XCanvasElement
    {
        ...
        // Debug mesh-data factory: single-group TriangleList wrapper for
        // ad-hoc debug geometry.  Positions / normals are required;
