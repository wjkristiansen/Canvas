...
namespace Canvas
{
    namespace Graphics
    {
        ...
        //------------------------------------------------------------------------------------------------
        // Buffers that are GPU accessible only.  Contains uniform
        // data such as constants or array-of-structures.
        ...
        {
        public:
            CDevice(CCanvas *pCanvas) :
                CObjectBase(pCanvas)
            {}
            GEMMETHOD(RenderFrame)() = 0;
            GEMMETHOD(AllocateUploadBuffer)(UINT64 SizeInBytes, Graphics::CUploadBuffer **ppUploadBuffer) = 0;
        };
