    ...
    *ppCanvas = nullptr;
    ...
    try
    {
        if (iid == XCanvas::IId)
        {
            if (pLogClient)
            {
                pLogClient->Write(QLog::LogCategory::Info, L"CANVAS", L"CreateCanvas: Creating canvas object...");
            }
    ...
    catch (std::bad_alloc &)
    {
        if (pLogClient)
        {
            pLogClient->Write(QLog::LogCategory::Error, L"CANVAS: ", L"FAILURE in CreateCanvas");
        }
        return Result::OutOfMemory;
    }
